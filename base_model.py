...
class ResNet(nn.Module):
    def __init__(self, pretrained=True,
        ...
        #     nn.Linear(in_features, 128), nn.ReLU(), nn.Dropout(p=0.2),
        #     # nn.Linear(1024, 1024), nn.ReLU(), nn.Dropout(p=0.2),
        #     # nn.Linear(128, num_classes)
        # )
        # self.wide = nn.Linear(in_features=11, out_features=16)
        # self.lin = nn.Linear(2064, 3)
        self.wide = nn.Linear(in_features=15, out_features=20)
        self.lin = nn.Linear(2068, 3)

    def forward(self, x, x1, mixup_lambda=None, perm=None):
        # do mix_up
        x = x.transpose(2, 3)
        # Run the convolutional blocks
        ...
        batch_size = x.size(0)
        # x = self.bn0(x)
        x_all = self.encoder(x).view(batch_size, -1)
        # x = self.bn1(x)
        # x_all = self.classifier(x)
        # add wide features and concat two layers
        x1 = self.wide(x1)
        x_all = torch.cat((x_all, x1), dim=1)
        x_all = self.lin(x_all)
